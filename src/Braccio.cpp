...
extern short basePos; 
extern short shoulderPos; 
extern short elbowPos; 
extern short wristRotPos; 
extern short wristVerPos; 
extern short handPos;
...
void _Braccio::turnBase(short angleValue, byte speed)
{
	...
	if (speed > 30)			speed		= 30;
  	elbowPos = angleValue;
  	ServoMovement(speed, angleValue, shoulderPos, elbowPos, wristRotPos, wristVerPos, gripperPos);
}


void _Braccio::turnShoulder(short angleValue, byte speed)
{
	if (angleValue < 20)   	angleValue 	= 20;
	if (angleValue > 160)  	angleValue 	= 160;
	if (speed < 10)			speed 		= 10;
	if (speed > 30)			speed		= 30;
	shoulderPos = angleValue;
	ServoMovement(speed, basePos, angleValue, elbowPos, wristRotPos, wristVerPos, gripperPos);
}

void _Braccio::turnElbow(short angleValue, byte speed)
{
	if (angleValue < 0)    	angleValue 	= 0;
	if (angleValue > 180)  	angleValue 	= 180;
	if (speed < 10)			speed 		= 10;
	if (speed > 30)			speed		= 30;
	elbowPos = angleValue;
	ServoMovement(speed, basePos, shoulderPos, angleValue, wristRotPos, wristVerPos, gripperPos);
}

void _Braccio::riseWristle(short angleValue, byte speed)
{
	if (angleValue < 0)    	angleValue 	= 0;
	if (angleValue > 180)  	angleValue 	= 180;
	if (speed < 10)			speed 		= 10;
	if (speed > 30)			speed		= 30;
	wristRotPos = angleValue;
	ServoMovement(speed, basePos, shoulderPos, elbowPos, angleValue, wristVerPos, gripperPos);
}

void _Braccio::turnHand(short angleValue, byte speed)
{
	if (angleValue < 0)    	angleValue 	= 0;
	if (angleValue > 180)  	angleValue 	= 180;
	if (speed < 10)			speed 		= 10;
	if (speed > 30)			speed		= 30;
	wristVerPos = angleValue;
  ServoMovement(speed, basePos, shoulderPos, elbowPos, wristRotPos, angleValue, gripperPos);
}

void _Braccio::openHand(short angleValue, byte speed)
{
	if (angleValue < 25)   	angleValue 	= 25;
	if (angleValue > 90)   	angleValue 	= 90;
	if (speed < 10)			speed 		= 10;
	if (speed > 30)			speed		= 30;
	gripperPos = angleValue;
 	ServoMovement(speed, basePos, shoulderPos, elbowPos, wristRotPos, wristVerPos, angleValue);
}

void _Braccio::handOpened(byte speed)
{
	gripperPos = 90;
	if (speed < 10)		speed 	= 10;
	if (speed > 30)		speed	= 30;
	openHand(gripperPos, speed);
}

void _Braccio::handClosed(byte speed)
{
	handPos = 25;
	if (speed < 10)		speed 	= 10;
	if (speed > 30)		speed	= 30;
	openHand(handPos, speed);
}